...
if(sum(ls(envir = .GlobalEnv) == "dm3S5AccessibilityProbability") == 0){
	loadDNAAccessibility=TRUE;
} else if(is.null(dm3S5AccessibilityProbability)){
	loadDNAAccessibility=TRUE;
}
...
if(loadDNAAccessibility){
	...
	if(file.exists("objects/Dm3S5AccessibilityProbability.RData")){
		load("objects/Dm3S5AccessibilityProbability.RData");
	} else{
...
if(toComputePWMScores){
	...
	if(file.exists("objects/BindingEnergyAtLociAccessibilityProbability.RData")){
		load("objects/BindingEnergyAtLociAccessibilityProbability.RData");
        } else{
		...
		for(i in 1:length(PWMThreshold)){
			...
			for(j in 1:nrow(lambdasMatrix)){
				bindingEnergy[[i]][[j]] = computePWMScores(directory="data", PFMFilename=PFMFilenames, PFMFormat="raw", DNASequenceSet=DNASequnceSet, setSequence=allSetPositive,
                                       BPFrequency=BPFrequency,TF=TF, DNAAccessibility=dm3S5AccessibilityProbability, PWMPseudocount=1,
		...
		save(bindingEnergy, file=paste("objects/BindingEnergyAtLociAccessibilityProbability.RData",sep=""));
...
if(extractDNAAccessibilityLoci){
	...
	DNAAccessibility=extractDNAAccesibilityAtLoci(dm3S5AccessibilityProbability,allSetPositive);
} else{
...
if(toComputeOccupancy){
	...
	if(file.exists("objects/occupancyAllTFsAllPositiveAccessibilityProbability.RData")){
		load("objects/occupancyAllTFsAllPositiveAccessibilityProbability.RData");
        } else{
		...
		save(occupancy, file=paste("objects/occupancyAllTFsAllPositiveAccessibilityProbability.RData",sep=""));
...
for(imageType in c("pdf")){
	for(i in 1:length(PWMThreshold)){
		...
		plotOccupancyModelQualityHeatmaps(occupancy=occupancy[[i]], TF=c("BCD","CAD"), lambdas=lambdasMatrix[,1], boundMolecules=boundMoleculesMatrix[,1], directory="img/heatmap",
						  plotFilename=paste("BCDCADAccessibiltiyProbability",txtCase,sep=""), imageType=imageType, contour=TRUE, regionsThreshold=0.12);
		plotOccupancyModelQualityHeatmaps(occupancy=occupancy[[i]], TF=c("GT","HB","KR"), lambdas=lambdasMatrix[,1], boundMolecules=boundMoleculesMatrix[,1], directory="img/heatmap",
						  plotFilename=paste("GTHBKRAccessibiltiyProbability",txtCase,sep=""), imageType=imageType, contour=TRUE, regionsThreshold=0.12);
	}
}
...
tableCaption="\\emph{Set of parameters that minimises the difference between the ChIP-seq profile in the case of continous accessibility data}. The analytical model includes continous DNA accessibility data by using equation \\eqref{eq:paccessible} to compute the probability that a site is accessible. We also listed the values for the mean squared error ($MSE$) and correlation ($\\rho$). The values in the parentheses represent the minimum mean squared error and the maximum correlation. We considered only the sites that have a PWM score higher than $70\\%$ of the distance between the lowest and the highest score.";	
tableLatex=xtable(optimalSetOfParameters, label ="tab:paramsModelAccProbability070", caption =tableCaption,align="|l|r|r|r|r|");
print.xtable(tableLatex, sanitize.text.function = function(x) x,file="tables/OptimalSetOfParametersTableAccessibilityProbability.tex",hline.after=-1:nrow(optimalSetOfParameters),floating.environment="table")
  ...
  for(i in 1:length(PWMThreshold)){
    for(TFid in c(2,5)){
      ...
      plotOccupancyProfile(occupancy=occupancy[[i]][[l]][[m]]$occupancyAbundanceChIP[[TFid]], PWMScore=occupancy[[i]][[l]][[m]]$occupancyAbundance[[TFid]], lambda=lambdasMatrix[l,TFid],
                           ...
                           plotFilename=paste(TF[TFid],"AccessibilityProbability",txtCase,sep=""), profile=profile[[TFid]], imageType=imageType, setSequence=allSetPositive,
