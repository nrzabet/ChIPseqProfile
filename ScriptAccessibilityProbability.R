		...
		for(i in 1:length(PWMThreshold)){
			...
			for(j in 1:nrow(lambdasMatrix)){
				bindingEnergy[[i]][[j]] = computePWMScores(directory="data", PFMFilename=PFMFilenames, PFMFormat="raw", DNASequenceSet=DNASequnceSet, setSequence=allSetPositive,
									   TF=TF, DNAAccessibility=dm3S5AccessibilityProbability, PWMPseudocount=1,
...
for(imageType in c("pdf")){
	for(i in 1:length(PWMThreshold)){
		...
		plotOccupancyModelQualityHeatmaps(occupancy=occupancy[[i]], TF=c("BCD","CAD"), lambdas=lambdasMatrix[,1], boundMolecules=boundMoleculesMatrix[,1], directory="img/heatmap",
						  plotFilename=paste("BCDCADAccessibiltiyProbability",txtCase,sep=""), imageType=imageType, contour=TRUE);
		plotOccupancyModelQualityHeatmaps(occupancy=occupancy[[i]], TF=c("GT","HB","KR"), lambdas=lambdasMatrix[,1], boundMolecules=boundMoleculesMatrix[,1], directory="img/heatmap",
						  plotFilename=paste("GTHBKRAccessibiltiyProbability",txtCase,sep=""), imageType=imageType, contour=TRUE);
	}
}
...
tableCaption="\\emph{Set of parameters that minimises the difference between the ChIP-seq profile in the case of continous accessibility data}. The analytical model includes continous DNA accessibility data by using equation \\eqref{eq:paccessible} to compute the probability that a site is accessible. We also listed the values for the mean square error ($MSE$) and correlation ($\\rho$). The values in the parentheses represent the minimum mean square error and the maximum correlation. We considered only the sites that have a PWM score higher than $70\\%$ of the distance between the lowest and the highest score.";	
tableLatex=xtable(optimalSetOfParameters, label ="tab:paramsModelAccProbability070", caption =tableCaption,align="|l|r|r|r|r|r|");
